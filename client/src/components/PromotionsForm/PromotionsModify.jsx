...
function PromotionsModify(props) {
  ...
  const handleSubmit = function (event) {
    ...
    swal(
      "Éxito!",
      `La promocion ${input.combo} ha sido modificada`,
      "success"
  ...
  function productFilterFromCategory(products, categoryCheckeds) {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (categoryProduct) {
        let filtered = categoryProduct.some((item) =>
          categoryCheckeds.includes(item)
        );
  ...
  useEffect(() => {
    ...
    if (productGlobal[0] && !productGlobal[0]?.error) {
      setInput({
        ...
        products: [
          ...input.products?.filter((n) => n.id !== productGlobal[0].id),
          productGlobal[0],
        ],
      });
  ...
  const handleDeleteProduct = function (e, id) {
    setInput({
      ...input,
      products: input.products.filter((p) => p.id !== id),
    });
  };
  //BUSCAR PRODUCTO
              ...
              return (
                <div className="ProductName">
                  ...
                  <CancelIcon
                    ...
                    onClick={(e) => handleDeleteProduct(e, n.id)}
                  />
                </div>
              );
            })}
