...
function CategoryFilter() {
  ...
  const categoryFiltered = useSelector(
    (state) => state.categoryFilterReducer.categoryFiltered
  );
  ...
  return (
    <div className="containerFilter">
      ...
      {categories.map((category, index) => {
        return categoryFiltered === category.name ? (
          <div
            className="categoriesLoaded  activeCategory"
            key={index}
